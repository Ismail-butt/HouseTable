...
const { getTodayDate, getSecondDate, getStringDate, getNumOfWeeks, validCurrency, getFee } = require('../utils/utils')

// @desc Get all appointments
// @route GET /api/appointments
// @acess Public
const getAppointments = asyncHandler(async (req, res) => {
    const appointments = await Appointment.find({}).populate('patient', 'name').sort('date')
...
const updateAppointment = asyncHandler(async (req, res) => {
    ...
    if (!patient) {
        res.status(404)
        throw new Error("The patient with given ID doesn't exist. Try to send Valid Patient Id")
    }
...
const getRemainingBill = asyncHandler(async (req, res) => {
    ...
    if (req.query.currency && !validCurrency(req.query.currency)) {
        res.status(404)
        throw new Error('You can only get Bill in Euro or USD. So, try to send to Valid Currency.')
    }
